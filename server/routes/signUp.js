...
const existsEmail = async (req, res, next) => {
  ...
  try {
    ...
    if (!result.length) {
      ...
      console.log(result);
      return res.status(403).send('すでに登録されてます');
    }
  } catch (err) {
    console.log(err);
...
router.post('/pre', existsEmail, async (req, res, next) => {
  ...
  try {
    ...
    transporter.sendMail(mailOptions1, function(error, info) {
      if (error) {
        console.error(error);
        return res.status(403).send('仮登録メールを送れませんでした');
      }
...
router.post('/auth', isLoggedIn, existsEmail, async (req, res, next) => {
  ...
  try {
    ...
    if (!resultAuth.length) {
      ...
      return res.status(403).send('番号かメアド間違えてまーす');
    }
    ...
    if (!resultEXP.length) {
      ...
      return res.status(403).send('期限がきれてまーす');
    }
